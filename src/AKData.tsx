...
export class AKData {
  ...
  featuredWait(op: Operator): number {
    if (op[this._region].featured.length > 0) {
      return Math.max(0, daysSince(op[this._region].featured[op.EN.featured.length - 1].end));
    }
    return daysSince(op[this._region].released);
  }

  shopWait(op: Operator): number {
    if (op[this._region].shop.length > 0) {
      return Math.max(0, daysSince(op[this._region].shop[op.EN.shop.length - 1].end));
    }
    return daysSince(op[this._region].released);
  }
  ...
  getFeaturedAverage(rarity: number): number {
    let sum = 0;
    let count = 0;
  ...
  getShopAverage(rarity: number): number {
    let sum = 0;
    let count = 0;
    this._sortedOperators
        .filter(op => op.rarity === rarity && !op.limited)
